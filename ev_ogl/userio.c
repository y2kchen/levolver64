...
#endif
		if (fgetc(l_brfd)!=EOF)
          /*Handle control-c, still need to determine best procedure here*/
          {
          ;//?!
          }
        }
...
{ char *c;
  ...
  if ( l_evolver_mode )
    {
/* This next section is simply a copy of the code for standard input,
    and most likely will be problematic if input is at the end of file

    On the other hand I don't understand the normal function - it looks
    like it demands input and does NOT wait for a typical delayed
    response from the console

    if ( my_fgets(inmsg,max,l_infd) == NULL )
       my_exit(0);
    c = inmsg + strlen(inmsg) - 1;
    if ( *c == '\n' ) *c = 0;
    if ( echo_flag ) { outstring(inmsg);  outstring("\n"); }
*/
    while ( my_fgets(inmsg,max,l_infd) == NULL )
       l_check_for_break(); /*Sleep(100); here?*/
/* WARNING: the prior statement could essentially be an infinite loop*/
    c = inmsg + strlen(inmsg) - 1;
    if ( *c == '\n' ) *c = 0;
    outstring(inmsg);
...
/* WARNING: the prior statement could essentially be an infinite loop*/
  ...
  //else /* Otherwise continue with normal standard input from console...*/
  ...
  outstring(promptmsg);
  quiet_flag = oldquiet;
/* __L_EVOLVER__:  This next function call may be better placed elsewhere*/
...
#ifdef __L_EVOLVER__
//else if (l_evolver_mode && cfd == l_infd &&  read_depth )
	 //read_depth = 0;
