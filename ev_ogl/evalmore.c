...
{ int i,j,n,k;
  ...
  switch ( node->type ) 
  {
    ...
    case PAUSE_NODE:
       ...
       if ( l_evolver_mode )
       {
		  outstring("Paused; Select OpenGL Window, type F3 to continue, F7 to break: ");       
          setOpenGL_key();
		  do {OpenGL_key = getOpenGL_key();} while ( OpenGL_key == LEVOLVER_WAITING_FOR_OPENGL_KEYPRESS);
		  if (OpenGL_key == LEVOLVER_BREAK_OPENGL_KEYPRESS)
             breakflag = BREAKFULL;
		  break;
	   }
#endif
...
#else
	#ifdef __L_EVOLVER__
		   /* If redirection failed and file layer mode was active restore functionality */
		   if (l_redirect)
		   {
			   l_redirect = FALSE;
			   l_evolver_mode = TRUE;
  ...
  switch ( node->type )
  {
   ...
   case SHOW_ALL_EDGES_NODE:
    ...
    case BINARY_PRINTFHEAD_NODE:
          ...
          outstring(msg);
#ifdef __L_EVOLVER__
		  levolvertemp = strncmp("Saving png\n", s, 10); /* comparing characters before new line*/
		  if ( levolvertemp==0 )
			  TakeScreenshot(0);
		  else
		  {
			  levolvertemp = strncmp("Set png prefix=", s, 15); 
			  if ( levolvertemp==0 )
			  {
				  strcpy(png_prefix, s+15);
				  /* Erase unneeded new line character if present in filename */
				  levolvertemp = strlen(png_prefix);
				  if ( levolvertemp > 0 && png_prefix[levolvertemp - 1] == '\n' )
					  png_prefix[levolvertemp - 1] = '\0' ;
			  }
			  else
			  {
				  levolvertemp = strncmp("Set png name=", s, 13); 
				  if ( levolvertemp==0 )
				  {
					  strcpy(png_name, s+13);
					  /* Erase unneeded new line character if present in filename */
					  levolvertemp = strlen(png_name);
					  if ( levolvertemp > 0 && png_name[levolvertemp - 1] == '\n' )
						  png_name[levolvertemp - 1] = '\0' ;
					  ...
					  {
						  strcpy(png_full_name, png_dir); 
						  strcat(png_full_name, png_name);
					  }
				  }
			  }			
		  }
