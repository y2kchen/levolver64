...
#ifdef __L_EVOLVER__
  char *plevolverpath=NULL , levolverpath[1024];
  char lfname[1024];   /*"\\Evolver\\communication\\"..*/
  extern void ActivatePngPathLogic(char *lfname);
  char *colon_character_pointer;
  char *colon_character_pointer2;
#endif
  msgmax = 2000; 
  ...
  if ( argc > 0 )
  {
     ...
     {    case 'U': 
          ...
          case 'f' : /* commands from file */
                 ...
                 break;
#ifdef __L_EVOLVER__
/* When Evolver starts with the command line option "-F=<Filename>" I/O goes
    through a file layer, i.e. Evolver attempts to access 3 unique files for
    input, output, and break commands (control-c). If needed this can be expanded
    to include a file that logs total_energy.

    This was created because there was some problems using pipes with FIT sending
    break commands, getting total_energy from output, and needing to log commands.

	When Evolver starts with the command line option "-G=<Title>" the graphics
	window will display the Title text rather then the filename
...
*/
		  case 'S':
			  ...
			  if( argv[0][2] = '=' )
			  //{
				 // ScreenSizeIndex = atoi(argv[0]+3);
			  //}
			    //modified by Y. Chen, 12/05/2011, for proper window position in the SE-FIT viewer
				{
				/* first find ':' , process last number, second number, then first number; */
				colon_character_pointer = strstr(argv[0]+3, ":");
				if (!colon_character_pointer)
					fputs("Levolver Error: missing : in command line option -S\n",stdout);
				else
					{
					colon_character_pointer2 = strstr(1+colon_character_pointer, ":");
					if (!colon_character_pointer2)
						fputs("Levolver Error: missing second : in command line option -S\n",stdout);
					else
						{
						*colon_character_pointer = 0;
						*colon_character_pointer2 = 0;
						x_pixel_width_cmd_line = atoi(++colon_character_pointer2);
						y_pixel_height_cmd_line = atoi(++colon_character_pointer);
						y_pixel_down_cmd_line = atoi(argv[0]+3);
						}
					}
				}
			  else    
          ...
          case 'X' : /* Program Break Delay (warning: case sensitive, x defined elsewhere)*/
			  if( argv[0][2] = '=' )
					{
					cProgramBreakDelay=0;

                    ProgramBreakDelay = atoi(argv[0]+3);
					}
                else    
                    {
                    fputs("Levolver Error: missing = in command line option -X\n",stdout);
                    }                   
          break;   
		  case 'G' : /* Graphics Window Title Text */
				if( argv[0][2] = '=' )
					{
					strcpy(GraphicsWindowTitleTextBuffer, argv[0]+3);
					GraphicsWindowTitleText = GraphicsWindowTitleTextBuffer;					
					}
                else    
                    {
                    fputs("Levolver Error: missing = in command line option -F\n",stdout);
                    }                   
          break;           

          case 'F' : /*  in, out, break for LEVOLVER */
				  plevolverpath=_getcwd(levolverpath,1024); 

				  if (plevolverpath == NULL)
					fputs("Levolver Error: current working directory name too long!\n",stderr);
				  else if( argv[0][2] = '=' )
                    {
                    l_evolver_mode = TRUE;

					strcpy(lfname, levolverpath);
#ifndef WIN32
					strcat(lfname, "/communication/in/");
#else
					strcat(lfname, "\\communication\\in\\");
#endif
					strcat(lfname, argv[0]+3);
                    l_infd = fopen(lfname, "r");
                    if ( !l_infd ) 
                        {
						l_evolver_mode = 0;
                        outstring("Levolver Error: Cannot Open -F=<file>");
						outstring(lfname);
						}
					strcpy(lfname, levolverpath);
#ifndef WIN32
					strcat(lfname, "/communication/out/");
#else
					strcat(lfname, "\\communication\\out\\");
#endif
					strcat(lfname, argv[0]+3);
                    l_outfd = fopen(lfname, "a");
                    if ( !l_outfd )  
                        {
						l_evolver_mode = 0;
                        outstring("Levolver Error: Cannot Open -F=<file>");
					...
					strcpy(lfname, levolverpath);
#ifndef WIN32
					strcat(lfname, "/communication/br/");
#else
					strcat(lfname, "\\communication\\br\\");
#endif
					strcat(lfname, argv[0]+3);
                    l_brfd = fopen( lfname, "r");
                    if ( !l_brfd ) 
                        {
						l_evolver_mode = 0;
                        outstring("Levolver Error: Cannot Open -F=<file>");
                    ...
                    if ( !l_infd || !l_outfd || !l_brfd ) 
                        {
						fputs("Failed to enter I/O layer mode",stdout);
                        my_exit(0);
/*kb_error(1349,"Broken pipe.\n",RECOVERABLE); 
*/
                        }
					else
						{
						//prepare string to represent png path
							
						strcpy(lfname, levolverpath);
#ifndef WIN32
						strcat(lfname, "/communication/history/png_path.txt");
#else
						strcat(lfname, "\\communication\\history\\png_path.txt");
#endif
						ActivatePngPathLogic(lfname);
						fputs("Successfully entered I/O layer mode\n",stdout);						
						}
                    }
                else    
                    {
